...
def make_rag_patches(func):
    """Stack the three patches every RAG test needs."""
    func = patch("backend.rag.retriever.search_wikipedia", new_callable=AsyncMock)(func)
    func = patch("backend.rag.retriever.fetch_article",    new_callable=AsyncMock)(func)
    func = patch("backend.services.llm.generate_answer",   return_value=MOCK_ANSWER)(func)
    return func
...
@pytest.mark.anyio
@patch("backend.services.llm.generate_answer", return_value=MOCK_ANSWER)
...
async def test_ask_caching(mock_search, mock_fetch, mock_generate, client):
    ...
    assert mock_generate.call_count == 1   # LLM called once; 2nd hit from cache
