...
def clear_cache():
    """Clear cache before every test to avoid state leaking between tests."""
    cache._store.clear()
    yield
    cache._store.clear()
...
async def test_ask_caching(mock_llm, mock_fetch, mock_search, client):
    ...
    # use a unique question so no other test pollutes this cache key
    ...
    # LLM should only be called once — second hit served from cache
    assert mock_llm.chat.completions.create.call_count == 1

@pytest.mark.anyio
@patch("backend.rag.retriever.retrieve_context", new_callable=AsyncMock)
async def test_ask_retrieval_failure(mock_retrieve, client):
    # patch retrieve_context at the point it's called inside main.py
    mock_retrieve.side_effect = Exception("Wikipedia API down")

    with patch("backend.main.retrieve_context", new_callable=AsyncMock) as mock_main_retrieve:
        mock_main_retrieve.side_effect = Exception("Wikipedia API down")
        response = await client.post("/ask", json={"question": "What is Python?"})
