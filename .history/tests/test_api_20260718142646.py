...
async def test_ask_success(mock_generate, mock_fetch, mock_search, client):
...
@patch("backend.rag.retriever.fetch_article", new_callable=AsyncMock)
@patch("backend.main.generate_answer", return_value=MOCK_ANSWER)
async def test_ask_returns_session_id(mock_generate, mock_fetch, mock_search, client):
    mock_search.return_value = ["Python"]
    mock_fetch.return_value = "Python is a high-level programming language. " * 20
...
@patch("backend.rag.retriever.fetch_article", new_callable=AsyncMock)
@patch("backend.main.generate_answer", return_value=MOCK_ANSWER)
async def test_ask_with_history(mock_generate, mock_fetch, mock_search, client):
    mock_search.return_value = ["Python"]
    mock_fetch.return_value = "Python is a high-level programming language. " * 20
...
@patch("backend.rag.retriever.fetch_article", new_callable=AsyncMock)
@patch("backend.main.generate_answer", return_value=MOCK_ANSWER)
async def test_ask_caching(mock_generate, mock_fetch, mock_search, client):
    mock_search.return_value = ["Python"]
    mock_fetch.return_value = "Python is a high-level programming language. " * 20
    ...
    # generate_answer called only once — second response from cache
