...
async function ask() {
  ...
  try {
    ...
    thinking.remove();
    const isNotFound = e.message.includes("404") || e.message.includes("No Wikipedia");
    const msg = isNotFound
      ? "⚠️ No Wikipedia article found for that query. Try rephrasing or using more specific keywords."
      : `⚠️ Error: ${e.message}`;
    addMessage("bot", `<span style="color:#b45309">${msg}</span>`);
  }
